...
void GLExtensionScanner::scanGLXExtensions(Display * d, int32_t s) {
    ...
    for(curEnd = extStr; *curEnd; curEnd++) {
        if(*curEnd == ' ') {
            index = getExtensionIndex(extStr);
...
void GLExtensionScanner::scanGLExtensions() {
    ...
    if(glMajor > 1 || (glMajor == 1 && glMinor >= 0)) extSupported[646] = GL_TRUE;
    if(glMajor > 1 || (glMajor == 1 && glMinor >= 1)) extSupported[647] = GL_TRUE;
    if(glMajor > 1 || (glMajor == 1 && glMinor >= 2)) extSupported[648] = GL_TRUE;
    if(glMajor > 1 || (glMajor == 1 && glMinor >= 3)) extSupported[649] = GL_TRUE;
    if(glMajor > 1 || (glMajor == 1 && glMinor >= 4)) extSupported[650] = GL_TRUE;
    if(glMajor > 1 || (glMajor == 1 && glMinor >= 5)) extSupported[651] = GL_TRUE;
    if(glMajor > 2 || (glMajor == 2 && glMinor >= 0)) extSupported[652] = GL_TRUE;
    if(glMajor > 2 || (glMajor == 2 && glMinor >= 1)) extSupported[653] = GL_TRUE;
    if(glMajor > 3 || (glMajor == 3 && glMinor >= 0)) extSupported[654] = GL_TRUE;
    if(glMajor > 3 || (glMajor == 3 && glMinor >= 1)) extSupported[655] = GL_TRUE;
    if(glMajor > 3 || (glMajor == 3 && glMinor >= 2)) extSupported[656] = GL_TRUE;
    if(glMajor > 3 || (glMajor == 3 && glMinor >= 3)) extSupported[657] = GL_TRUE;
    if(glMajor > 4 || (glMajor == 4 && glMinor >= 0)) extSupported[658] = GL_TRUE;
    if(glMajor > 4 || (glMajor == 4 && glMinor >= 1)) extSupported[659] = GL_TRUE;
    if(glMajor > 4 || (glMajor == 4 && glMinor >= 2)) extSupported[660] = GL_TRUE;
    if(glMajor > 4 || (glMajor == 4 && glMinor >= 3)) extSupported[661] = GL_TRUE;
    if(glMajor > 4 || (glMajor == 4 && glMinor >= 4)) extSupported[662] = GL_TRUE;
    if(glMajor > 4 || (glMajor == 4 && glMinor >= 5)) extSupported[663] = GL_TRUE;
    if(glMajor > 4 || (glMajor == 4 && glMinor >= 6)) extSupported[664] = GL_TRUE;
    ...
    if(glMajor >= 3) {
        ...
        if(!(_glGetStringi = (PFNGLGETSTRINGIPROC)glXGetProcAddressARB(
                (const GLubyte *)"glGetStringi"))) return;
        GLint numExts;
        glGetIntegerv(GL_NUM_EXTENSIONS, &numExts);
        for(int32_t i = 0; i < numExts; i++) {
            extStr = (const char *)_glGetStringi(GL_EXTENSIONS, i);
            index = getExtensionIndex(extStr);
            if(index != UINT16_MAX) extSupported[index] = GL_TRUE;
        }
    } else {
...
uint16_t GLExtensionScanner::getExtensionIndex(const char * extStr) {
    ...
    while(true) {
        mid = (hi-lo)/2+lo;
        res = compareExtensionStrings(extStr, extensions[mid]);
        if(res < 0) hi = mid-1;
...
bool GLExtensionScanner::isExtensionSupported(const char * ext) {
    uint16_t index = getExtensionIndex(ext);
    if(index == UINT16_MAX) return false;
    else return extSupported[index];
}

void GLExtensionScanner::getGLVersion(GLint & major, GLint & minor) {
    major = glMajor;
    minor = glMinor;
}

void GLExtensionScanner::getGLXVersion(int32_t & major, int32_t & minor) {
    major = glXMajor;
    minor = glXMinor;
}
